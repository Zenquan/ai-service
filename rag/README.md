...
**检索链路**：混合检索（关键词 BM25 式 + 向量相似 → RRF 融合）→ bge-reranker 精排 → 引用校验。
...
main.py: ingest →  解析(MinerU 云→markitdown→兜底) → 清洗 → 切块 → FastEmbed 向量 → Qdrant 入库
                     │
                     ▼
main.py: ask   →  query 向量化 → Qdrant 相似检索 topK
      →（可选 rerank）→ 注入 Prompt → DeepSeek 生成 [来源N] 回答 → 引用校验
```

## 快速开始

```bash
# 1. 安装依赖（Python 3.12，⚠️ 3.13/3.14 无 fastembed 依赖 wheel；3.10-3.12 均可）
cd rag
...
- **免 Docker**：Qdrant 用 local 模式（`path=`），代码与生产 `:memory:`/远端同一套 API
...
- **混合检索**（P1）：关键词（中文 2-gram + 英文词）+ 向量双路召回 → **RRF 融合**（术语精确命中 + 语义扩展兼顾），可 `use_mixed=False` 关
...
### 1. Python 3.14 装不了 fastembed
- **现象**：`pip install fastembed` 报依赖冲突（onnxruntime/tokenizers 在 3.14 无 wheel）
- **修复**：用 Python 3.12 重建 venv（fastembed 官方支持 3.8-3.12）
- **要点**：AI/ML 生态对新 Python 版本支持滞后，"先查依赖 wheel 再选 Python 版本"
...
- **修复**：命令串行执行；`get_client()` 加了中文友好提示；要并发需换 Qdrant server/Docker
...
- **现象**：`exp_chunk_size.py` 跑 400/800/1500/3000 四组，命中率全是 100%，无法区分参数好坏
- **根因**：初版评估是"关键词是否出现在 topK 拼接文本里"（模糊命中）——条件太宽松，任何 size 都能命中
- **修复**：升级为"**期望文档命中（expect_doc）**"——判断检索结果是否来自正确文档（Recall@K 的简化版），区分度立刻出来
- **要点**：**评估集要先能区分好坏，否则调参是盲调**——这是"用评测驱动"的第一步
...
- [x] 评估升级：7 用例 `expect_doc + expect_kw` 双条件，命中率 7/7 = 100%
...
**评估升级（配套）**：`eval_cases.json` 从关键词匹配 → `expect_doc`（期望命中文档）+ `expect_kw`（关键词），`main.evaluate()` 双条件判定，让调参有区分度。

## 优化记录（v3 · P1 检索 + 工程三项）

| 优先级 | 项 | 做了什么 | 收益 |
|--------|-----|---------|------|
| P1 | **混合检索** | `retrieve.py`：关键词（中文 2-gram + 英文词，BM25 式命中计数）+ 向量双路召回 → **RRF 融合**（`_rrf_merge`，按排名倒数加权）→ rerank | 术语精确命中（编号/专名）+ 语义扩展（同义/多语言）兼顾，互补召回 |
