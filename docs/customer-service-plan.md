...
Phase 0 已完成首个可运行闭环，代码位于 `langgraph/src/langgraph_graph/customer_service/`：

- `CustomerServiceState` 固化会话、意图、槽位、检索上下文、引用和人工接管字段。
- 确定性分类器覆盖问候、知识问答、订单、售后、投诉和未知请求，后续可替换为 LLM 分类器。
...
- 向量 + BM25 + RRF 混合检索。
- 可选 rerank、引用校验和 Recall@K/MRR 离线评测。
- 独立的 LangGraph 实验目录，已有 retrieve、rerank、generate、validate 节点。
