...
- 标注集字段：`question` / `intent` / `expected_tools` / `required_slots` / `must_handoff` / `forbidden_actions` / `user_id`；内置 8 条样例，支持 `--cases` 指定 JSON。
...
- 客服任务离线评测集：`offline_eval.py`（`evaluate_customer_service` + `cs-eval` CLI），mock 工具跑图验证意图/工具/转人工/越权/槽位五项任务级正确性，内置 8 条标注样例。
...
3. **离线评测集扩充**：标注集扩到真实场景语料，与 Recall@K/MRR 检索评测联动、纳入 CI 回归。
